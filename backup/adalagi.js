...
//end
    ...
    const pickRandom = (arr) => {
          ...
          // --- DATABASE ANONYMOUS (Letakkan di luar asya = async...) ---
if (global.anonymous_chat === undefined) global.anonymous_chat = {};

// --- LOGIKA JEMBATAN ANONYMOUS (MULTIMEDIA SUPPORT) ---
const partnerJid = global.anonymous_chat[m.sender];
if (partnerJid && !isCmd) {
    try {
        if (m.mtype === 'conversation' || m.mtype === 'extendedTextMessage') {
            // KIRIM TEKS BIASA
            await asya.sendMessage(partnerJid, { text: m.text });
        } else {
            // PROSES SEMUA MEDIA (Foto, Video, Sticker, VN, Audio, Dokumen)
            let media = await asya.downloadMediaMessage(m.msg || m);
            
            if (m.mtype === 'stickerMessage') {
                await asya.sendImageAsSticker(partnerJid, media, null, { packname: "Anonymous Chat", author: "JOJO-BOT" });
            } else if (m.mtype === 'audioMessage') {
                await asya.sendMessage(partnerJid, { audio: media, mimetype: 'audio/mp4', ptt: m.msg.ptt });
            } else {
                let type = m.mtype.replace('Message', '');
                await asya.sendMessage(partnerJid, { 
                    [type]: media, 
                    caption: m.msg.caption || '', 
                    mimetype: m.msg.mimetype,
                    fileName: m.msg.fileName || `file.${m.msg.mimetype?.split('/')[1]}`
                });
            }
        }
    } catch (err) {
        console.error("Error Forwarding Anonymous:", err);
        asya.sendMessage(m.sender, { text: "❌ Gagal meneruskan media. Pastikan file tidak terlalu besar." });
    }
    return; // Stop agar tidak terbaca sebagai command lain
}
...
Contoh : 🔔 : 🔔 : 🔔`
    ...
    /// ANON
    case 'start': {
    if (global.anonymous_chat[sender]) return reply('Anda sudah berada dalam sesi chat!');
    
    // Cari user lain yang sedang waiting (tatus: waiting)
    let waitingUser = Object.keys(asya.anonymous || {}).find(id => asya.anonymous[id].status === 'waiting' && id !== sender);

    if (waitingUser) {
        // Partner Ditemukan
        global.anonymous_chat[sender] = waitingUser;
        global.anonymous_chat[waitingUser] = sender;
        delete asya.anonymous[waitingUser]; // Hapus dari daftar tunggu

        const txtKet = `*Partner ditemukan!* 🎉\n\nKamu sekarang bisa mengirim Teks, Gambar, Video, Sticker, maupun VN secara anonim. Identitasmu terjaga!`;
        const btnAnon = [
            { text: 'Cari Baru 🔄', id: `${prefix}next` },
            { text: 'Stop Chat ❌', id: `${prefix}stop` }
        ];

        // Kirim ke Pengirim
        await sendButton(sender, txtKet, jam(), btnAnon, m);
        // Kirim ke Partner
        await sendButton(waitingUser, txtKet, jam(), btnAnon, m);
    } else {
        // Masuk Antrian
        if (!asya.anonymous) asya.anonymous = {};
        asya.anonymous[sender] = { id: sender, status: 'waiting' };
        reply('🔍 Mencari partner anonymous... Mohon tunggu sebentar.\n\nKetik */stop* jika ingin membatalkan.');
    }
}
break;

case 'stop': {
    const partner = global.anonymous_chat[sender];
    if (partner) {
        await asya.sendMessage(partner, { text: '_Partner telah menghentikan obrolan._' });
        delete global.anonymous_chat[partner];
    }
    if (asya.anonymous && asya.anonymous[sender]) delete asya.anonymous[sender];
    delete global.anonymous_chat[sender];
    reply('Sesi chat dihentikan.');
}
break;
...
case 'next': {
    const partner = global.anonymous_chat[sender];
    if (partner) {
        await asya.sendMessage(partner, { text: '_Partner telah mengakhiri obrolan._' });
        delete global.anonymous_chat[partner];
    }
    delete global.anonymous_chat[sender];
    // Otomatis cari lagi
    asya.ev.emit('messages.upsert', { messages: [{ ...m, body: `${prefix}start` }], type: 'notify' });
}
break;
            case 'sticker':
    case 'stiker':
